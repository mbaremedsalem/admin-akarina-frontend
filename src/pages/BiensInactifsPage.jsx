...
export default function BiensInactifsPage() {
  ...
  const inactiveBiens = useMemo(() => biens.filter((row) => !row.actif), [biens]);
  ...
  const [modal, setModal] = useState(null); // { mode: 'edit', initial, reference }
  const [loadingDetail, setLoadingDetail] = useState(false);
  const [search, setSearch] = useState("");

  const filteredBiens = useMemo(() => {
    if (!search.trim()) return inactiveBiens;
    const q = search.trim().toLowerCase();
    return inactiveBiens.filter((row) => JSON.stringify(row).toLowerCase().includes(q));
  }, [inactiveBiens, search]);
  ...
  return (
    <div className="resource-page">
      <div className="page-header">
        <h1>
          <span className="page-header-icon">
            <Ban className="icon" strokeWidth={2} />
          </span>
          {t("biens.titleInactifs")}
        </h1>
        <div className="page-header-actions">
          {inactiveBiens.length > 0 && <SearchToggle value={search} onChange={setSearch} />}
        </div>
      </div>
      ...
      ) : inactiveBiens.length === 0 ? (
        <div className="empty-state">
          <Ban className="icon" />
          {t("biens.emptyInactifs")}
        </div>
      ...
      {modal && (
        <Modal
          title={t("biens.editBien", { reference: modal.reference })}
